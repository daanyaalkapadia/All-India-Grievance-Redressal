...
router.get("/redressing/:user/:complaint/:redirection",ensureAuthenticated,function(req,res){
    ...
    if(redirect == "pending")
    {
        ...
        res.redirect("/../users/viewcomplaint/"+userId+"/"+complaintId);
    }
});

//View Complaint
router.get("/viewcomplaint/:user/:complaint",ensureAuthenticated,function(req,res){
    var userId = req.params.user;
    var complaintId = req.params.complaint;
    ref.once("value", function(snapshot) {
        var allUsers = snapshot.val();
        userName = allUsers[userId].fname +" "+allUsers[userId].lname
